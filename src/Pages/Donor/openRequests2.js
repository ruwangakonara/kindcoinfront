    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            const filtered = requests.filter(request => {
                const titleMatch = request.requestDetails.title.toLowerCase().includes(searchQuery.toLowerCase());
                if(titleMatch){
                    console.log("the tkitlematches")
                }

                const descriptionMatch = request.requestDetails.description.toLowerCase().includes(searchQuery.toLowerCase());
                const addressMatch = request.requestDetails.address.toLowerCase().includes(searchQuery.toLowerCase());
                const beneficaryMatch = request.name.toLowerCase().includes(searchQuery.toLowerCase());
                const typeMatch = request.requestDetails.type.toLowerCase().includes(searchQuery.toLowerCase());

                const beneDesMatch = request.description.toLowerCase().includes(searchQuery.toLowerCase());
                console.log("buns")
                ...
                return titleMatch || descriptionMatch ||
                       entityMatchdescription || beneficaryMatch || typeMatch ||
                       entityMatchTitle || entityMatchTitle || entityMatchType
                        || entityMatchName || beneDesMatch || entityMatchBeneDes
                        || addressMatch  || entityMatchAddress
